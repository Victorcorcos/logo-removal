...
class LogoRemover:
    ...
    def process_image(self, input_path: str, output_path: str) -> str:
        ...
        try:
            ...
            if self.detector.owl_classifier is not None:
                has_watermark, confidence = self.detector.has_watermark_owl(pil_image)
            ...
            # Detect logos with YOLO (using multi-angle detection)
            boxes = self.detector.detect(input_path, use_multiangle=True)

            if len(boxes) == 0:
                if self.detector.owl_classifier is not None and has_watermark:
...
Examples:
  ...
  python remove_logos.py -i /path/to/images -c 0.5 -e 20
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.15,
        help='Confidence threshold for detection (0-1, default: 0.15)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.40,
        help='OWLv2 watermark detection threshold (0-1, default: 0.40)'
    )
